...
zend_persistent_script *zend_file_cache_script_load(zend_file_handle *file_handle)
{
	...
	if (ZCG(accel_directives).file_cache_consistency_checks &&
	    (actaul_checksum = zend_adler32(ADLER32_INIT, mem, info.mem_size + info.str_size)) != info.checksum) {
		zend_accel_error(ACCEL_LOG_WARNING, "corrupted file '%s' excepted checksum: 0x%08x actaul checksum: 0x%08x\n", filename, info.checksum, actaul_checksum);
		zend_file_cache_unlink(filename);
		zend_arena_release(&CG(arena), checkpoint);
		efree(filename);
		return NULL;
